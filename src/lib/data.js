export const skillData = [
  { subject: "Python", value: 92 },
  { subject: "SQL", value: 88 },
  { subject: "Machine Learning", value: 85 },
  { subject: "Statistika", value: 80 },
  { subject: "Visualisasi Data", value: 90 },
  { subject: "Business Acumen", value: 75 },
];

export const projects = [
  {
    title: "Dashboard Kekerasan di Jawa Barat",
    desc: "Membuat dashboard serta menganalisis kualitas pencatatan data di Jawa Barat dan melihat kekerasan apa yang paling banyak terjadi berdasarkan kota/kabupaten atau tahun.",
    metric: "5",
    metricLabel: "Dataset terpisah",
    tags: ["Excel", "Power BI"],
    thumbnail: "/thumbnails/project-1.png",
    githubUrl: "https://github.com/muharromganesha/Kekerasan-di-JawaBarat",
    mediumUrl: "https://medium.com/@MuharromGanesha/analisis-tren-dan-kualitas-data-pelaporan-kasus-kekerasan-di-jawa-barat-2018-2024-5cdc7e4ceea6?sharedUserId=MuharromGanesha",
    driveUrl: "",
  },
  {
    title: "Dashboard Penjualan Ritel",
    desc: "Menganalisis penjualan sebuah ritel fiktif untuk dilihat barang apa yang yang paling sering dibeli serta melihat perkembangan toko per kuarter.",
    metric: "1.6M+",
    metricLabel: "baris data",
    tags: ["Python (Pandas)", "Power BI"],
    thumbnail: "/thumbnails/project-2.png",
    githubUrl: "https://github.com/muharromganesha/Online-Retail-Sales-Analysis",
    mediumUrl: "https://medium.com/@MuharromGanesha/analisis-online-retail-sales-performance-2010-2011-4ec8da1a3637?sharedUserId=MuharromGanesha",
    driveUrl: "",
  },
  {
    title: "Webiste Pencatatan Keuangan Pribadi",
    desc: "On Going.",
    metric: "",
    metricLabel: "",
    tags: ["React.js", "Supabase", "Gemini API"],
    thumbnail: "/thumbnails/project-3.png",
    githubUrl: "",
    mediumUrl: "",
    driveUrl: "#",
  },
  {
    title: "Analisis Kecelakaan di Kota New York",
    desc: "Analisis seberapa parah kecelakaan berdasarkan nama jalan, serta faktor faktor apa saja yang menjadi penyebab kecelakaan terbanyak, beserta kelompok peserta MSIB Kampus Merdeka.",
    metric: "207K",
    metricLabel: "baris data",
    tags: ["Excel", "Python (Pandas)", "Looker"],
    thumbnail: "/thumbnails/project-4.png",
    githubUrl: "",
    mediumUrl: "",
    driveUrl: "https://drive.google.com/file/d/1NCti886hD3oVFf_tSlvhuZaBD8Ji6HEl/view?usp=sharing",
  },
];

export const experience = [
    {
      role: "Web Developer (Campus Internship)",
      company: "CV. Manunggal Jaya Abadi",
      period: "Sept. 2024 — Jan. 2025",
      desc: "Membuat aplikasi sistem berbasis Website untuk pemesanan di sebuah rumah makan di bawah naungan CV. Manuggal Jaya Abadi, bekerja sama dengan teman satu kelompok dan tim IT perusahaan.",
    },
];
